...
\begin{frame}{Case 2}{Heat conduction in an L-clamp}
    \begin{columns}
        \begin{column}{0.5\linewidth}
            \begin{figure}
                \centering
                \begin{tikzpicture}[x={(0.5cm,0.35cm)}, y={(-0.65cm,0.2cm)}, z={(0cm,0.75cm)}, >=stealth, inner sep=1pt, font=\scriptsize]
                    ...
                    \begin{scope}[canvas is xy plane at z=0]
                        \draw (0,0) node [below] {0} -- node [anchor=north west] {\qty{4}{m}} (4,0) node [below] {1} -- node [anchor=south west] {\qty{1}{m}} (4,1) node [anchor=south east] {2} -- (1,1) node [anchor=north west] {3} -- cycle;
